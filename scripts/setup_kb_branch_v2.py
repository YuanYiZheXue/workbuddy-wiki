...
# 硬编码主仓库路径，避免 __file__ 中文路径乱码
REPO_DIR = Path(r"d:\Obsidian_KN\知识库构建")
MAIN_REPO = "origin"
MAIN_BRANCH = "main"
...
def setup(branch_name, workspace_dir, kb_name, agent_name, emoji, force=False):
    ...
    # 【安全校验】workspace_dir 不能是仓库根目录，否则会误删一切
    if workspace.resolve() == REPO_DIR.resolve():
        print("[ERROR] workspace_dir 不能是仓库根目录！")
        print(f"  仓库根目录：{REPO_DIR}")
        print(f"  你传入了  ：{workspace}")
        sys.exit(1)
