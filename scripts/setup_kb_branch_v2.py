...
### v2 的问题
1. 克隆到临时目录再 move → Windows 跨磁盘（C:→D:）move 失败
   → 修复：直接 clone 到目标路径
2. .git 目录文件被锁定 → shutil.rmtree 失败
   → 修复：用 robocopy /MIR 清空目录后再删

### v3 的问题
1. 分支已存在时，git checkout 被未跟踪文件阻挡
   → 修复：git checkout -f 强制切换

### v4 的问题
1. 清理 wiki/ 时删除了子知识库索引目录（计算机/、数学/、哲学/）
   → 修复：只保留 index.md + 统筹/，用临时目录备份恢复

### v5 的问题
1. --force 会删除用户已有的 wiki/ 内容
   → 修复：只在分支【新创建】时清理 wiki/，已存在分支跳过
2. 工作空间已存在时直接报错退出
   → 修复：已存在 → git pull 增量更新，不重新 clone

### v6 的问题
1. 子知识库的 workbuddy-wiki-schema.md 是简化版，子 Agent 不知道怎么建 Wiki
   → 修复：复制主空间【完整版】schema，只改标题 + 加子知识库标注
2. scripts/ 只复制了 5 个脚本，不够全
   → 修复：复制 scripts/ 下所有 .py 文件

### v7（当前稳定版）
- 纯增量，零删除
- 子知识库 schema = 主空间完整版 + 标注
- 框架文件全量复制（scripts/、wiki/统筹/）
- 知识内容（wiki/概念、wiki/实体...）永远不碰
"""
